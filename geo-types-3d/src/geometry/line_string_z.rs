use crate::{CoordZ, CoordNum, LineZ, PointZ, Triangle};
use alloc::vec;
use alloc::vec::Vec;
...
#[derive(Debug)]
pub struct PointsIter<'a, T: CoordNum + 'a>(::core::slice::Iter<'a, CoordZ<T>>);

impl<T: CoordNum> Iterator for PointsIter<'_, T> {
    type Item = PointZ<T>;

    fn next(&mut self) -> Option<Self::Item> {
        self.0.next().map(|c| PointZ::from(*c))
    }
...
impl<T: CoordNum> DoubleEndedIterator for PointsIter<'_, T> {
    fn next_back(&mut self) -> Option<Self::Item> {
        self.0.next_back().map(|c| PointZ::from(*c))
    }
}

/// A [`Coord`] iterator used by the `into_iter` method on a [`LineString`]
#[derive(Debug)]
pub struct CoordinatesIter<'a, T: CoordNum + 'a>(::core::slice::Iter<'a, CoordZ<T>>);

impl<'a, T: CoordNum> Iterator for CoordinatesIter<'a, T> {
    type Item = &'a CoordZ<T>;
...
impl<T: CoordNum> LineStringZ<T> {
    ...
    pub fn coords(&self) -> impl DoubleEndedIterator<Item = &CoordZ<T>> {
        self.0.iter()
    }

    /// Return an iterator yielding the coordinates of a [`LineString`] as mutable [`Coord`]s
    pub fn coords_mut(&mut self) -> impl DoubleEndedIterator<Item = &mut CoordZ<T>> {
        self.0.iter_mut()
    }

    /// Return the coordinates of a [`LineString`] as a [`Vec`] of [`Point`]s
    pub fn into_points(self) -> Vec<PointZ<T>> {
        self.0.into_iter().map(PointZ::from).collect()
    }

    /// Return the coordinates of a [`LineString`] as a [`Vec`] of [`Coord`]s
    pub fn into_inner(self) -> Vec<CoordZ<T>> {
        self.0
    }
    ...
    /// ```
    pub fn lines(&'_ self) -> impl ExactSizeIterator<Item = LineZ<T>> + '_ {
        self.0.windows(2).map(|w| {
            // slice::windows(N) is guaranteed to yield a slice with exactly N elements
            unsafe { LineZ::new(*w.get_unchecked(0), *w.get_unchecked(1)) }
        })
    }
    ...
    /// assert!(lines.next().is_none());
    /// ```
    pub fn rev_lines(&'_ self) -> impl ExactSizeIterator<Item = LineZ<T>> + '_ {
        self.0.windows(2).rev().map(|w| {
            // slice::windows(N) is guaranteed to yield a slice with exactly N elements
            unsafe { LineZ::new(*w.get_unchecked(1), *w.get_unchecked(0)) }
        })
    }
...
impl<T: CoordNum, IC: Into<CoordZ<T>>> From<Vec<IC>> for LineStringZ<T> {
    fn from(v: Vec<IC>) -> Self {
        Self(v.into_iter().map(|c| c.into()).collect())
    }
}

impl<T: CoordNum> From<LineZ<T>> for LineStringZ<T> {
    fn from(line: LineZ<T>) -> Self {
        LineStringZ::from(&line)
    }
}
...
impl<T: CoordNum, IC: Into<CoordZ<T>>> FromIterator<IC> for LineStringZ<T> {
...
impl<T: CoordNum> IntoIterator for LineStringZ<T> {
    type Item = CoordZ<T>;
    type IntoIter = ::alloc::vec::IntoIter<CoordZ<T>>;
...
impl<'a, T: CoordNum> IntoIterator for &'a LineStringZ<T> {
    type Item = &'a CoordZ<T>;
...
impl<'a, T: CoordNum> IntoIterator for &'a mut LineStringZ<T> {
    type Item = &'a mut CoordZ<T>;
    type IntoIter = ::core::slice::IterMut<'a, CoordZ<T>>;

    fn into_iter(self) -> ::core::slice::IterMut<'a, CoordZ<T>> {
        self.0.iter_mut()
    }
}

impl<T: CoordNum> Index<usize> for LineStringZ<T> {
    type Output = CoordZ<T>;

    fn index(&self, index: usize) -> &CoordZ<T> {
        self.0.index(index)
    }
}

impl<T: CoordNum> IndexMut<usize> for LineStringZ<T> {
    fn index_mut(&mut self, index: usize) -> &mut CoordZ<T> {
...
mod approx_integration {
    ...
    impl<T> RelativeEq for LineStringZ<T>
    where
    ...
    impl<T> AbsDiffEq for LineStringZ<T>
    where
    ...
    impl<T> UlpsEq for LineStringZ<T>
    where
...
mod test {
    ...
    fn test_exact_size() {
        ...
        let first = coordZ! { x: 0., y: 0. , z: 0.};
        let ls = LineStringZ::new(vec![first, coordZ! { x: 10., y: 0., z: 0.}]);
    ...
    fn test_abs_diff_eq() {
        ...
        let coords = vec![(0., 0., 0.), (5., 0., 0.), (10., 10., 10.)];
        let ls: LineStringZ<f32> = coords.into_iter().collect();

        let coords_x = vec![(0., 0., 0.), (5. + delta, 0., 0.), (10., 10., 10.)];
        let ls_x: LineStringZ<f32> = coords_x.into_iter().collect();
        assert!(ls.abs_diff_eq(&ls_x, 1e-2));
        assert!(ls.abs_diff_ne(&ls_x, 1e-12));

        let coords_y = vec![(0., 0., 0.), (5., 0. + delta, 0.), (10., 10., 10.)];
        let ls_y: LineStringZ<f32> = coords_y.into_iter().collect();
        assert!(ls.abs_diff_eq(&ls_y, 1e-2));
        assert!(ls.abs_diff_ne(&ls_y, 1e-12));

        // Undersized, but otherwise equal.
        let coords_x = vec![(0., 0., 0.), (5., 0., 0.)];
        let ls_under: LineStringZ<f32> = coords_x.into_iter().collect();
        assert!(ls.abs_diff_ne(&ls_under, 1.));

        // Oversized, but otherwise equal.
        let coords_x = vec![(0., 0., 0.), (5., 0., 0.), (10., 10., 10.), (10., 100., 10.)];
        let ls_oversized: LineStringZ<f32> = coords_x.into_iter().collect();
    ...
    fn should_be_built_from_line() {
        let start = coordZ! { x: 0, y: 0, z: 0 };
        let end = coordZ! { x: 10, y: 10, z: 10 };
        let line = LineZ::new(start, end);
        let expected = LineStringZ::new(vec![start, end]);

        assert_eq!(expected, LineStringZ::from(line));

        let start = coordZ! { x: 10., y: 0.5, z: 0. };
        let end = coordZ! { x: 10000., y: 10.4, z:0. };
        let line = LineZ::new(start, end);
        let expected = LineStringZ::new(vec![start, end]);
    ...
    #[test]
    fn empty() {
        let empty = LineStringZ::<f64>::empty();
