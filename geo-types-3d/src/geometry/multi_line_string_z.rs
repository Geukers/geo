use crate::{CoordNum, LineStringZ};
...
impl<T: CoordNum> MultiLineStringZ<T> {
    ...
    pub fn new(value: Vec<LineStringZ<T>>) -> Self {
        Self(value)
    }
    ...
    ///
    /// ```
    /// use geo_types_3d::{MultiLineStringZ, LineStringZ, line_string_z};
    ///
    /// let open_line_string: LineStringZ<f32> = line_string_z![(x: 0., y: 0., z: 0.), (x: 5., y: 0., z: 0.)];
    /// assert!(!MultiLineStringZ::new(vec![open_line_string.clone()]).is_closed());
    ///
    /// let closed_line_string: LineStringZ<f32> = line_string_z![(x: 0., y: 0., z: 0.), (x: 5., y: 0., z: 0.), (x: 0., y: 0., z: 0.)];
    /// assert!(MultiLineStringZ::new(vec![closed_line_string.clone()]).is_closed());
    ///
    /// // MultiLineString is not closed if *any* of it's LineStrings are not closed
    /// assert!(!MultiLineStringZ::new(vec![open_line_string, closed_line_string]).is_closed());
    ///
    /// // An empty MultiLineString is closed
    /// assert!(MultiLineStringZ::<f32>::empty().is_closed());
    /// ```
    pub fn is_closed(&self) -> bool {
        // Note: Unlike JTS et al, we consider an empty MultiLineString as closed.
        self.iter().all(LineStringZ::is_closed)
    }
}

impl<T: CoordNum, ILS: Into<LineStringZ<T>>> From<ILS> for MultiLineStringZ<T> {
    fn from(ls: ILS) -> Self {
        Self(vec![ls.into()])
    }
}

impl<T: CoordNum, ILS: Into<LineStringZ<T>>> FromIterator<ILS> for MultiLineStringZ<T> {
    fn from_iter<I: IntoIterator<Item = ILS>>(iter: I) -> Self {
        Self(iter.into_iter().map(|ls| ls.into()).collect())
    }
}

impl<T: CoordNum> IntoIterator for MultiLineStringZ<T> {
    type Item = LineStringZ<T>;
    type IntoIter = ::alloc::vec::IntoIter<LineStringZ<T>>;
...
impl<'a, T: CoordNum> IntoIterator for &'a MultiLineStringZ<T> {
    type Item = &'a LineStringZ<T>;
    type IntoIter = ::alloc::slice::Iter<'a, LineStringZ<T>>;
...
impl<'a, T: CoordNum> IntoIterator for &'a mut MultiLineStringZ<T> {
    type Item = &'a mut LineStringZ<T>;
    type IntoIter = ::alloc::slice::IterMut<'a, LineStringZ<T>>;
...
impl<T: CoordNum> MultiLineStringZ<T> {
    pub fn iter(&self) -> impl Iterator<Item = &LineStringZ<T>> {
        self.0.iter()
    }

    pub fn iter_mut(&mut self) -> impl Iterator<Item = &mut LineStringZ<T>> {
        self.0.iter_mut()
    }
}
...
impl<T: CoordNum + Send> IntoParallelIterator for MultiLineStringZ<T> {
    type Item = LineStringZ<T>;
    type Iter = rayon::vec::IntoIter<LineStringZ<T>>;
...
#[cfg(feature = "multithreading")]
impl<'a, T: CoordNum + Sync> IntoParallelIterator for &'a MultiLineStringZ<T> {
    type Item = &'a LineStringZ<T>;
    type Iter = rayon::slice::Iter<'a, LineStringZ<T>>;
...
#[cfg(feature = "multithreading")]
impl<'a, T: CoordNum + Send + Sync> IntoParallelIterator for &'a mut MultiLineStringZ<T> {
    type Item = &'a mut LineStringZ<T>;
    type Iter = rayon::slice::IterMut<'a, LineStringZ<T>>;
...
mod approx_integration {
    ...
    impl<T> RelativeEq for MultiLineStringZ<T>
    where
    ...
    impl<T> AbsDiffEq for MultiLineStringZ<T>
    where
    ...
    impl<T> UlpsEq for MultiLineStringZ<T>
    where
...
mod test {
    use super::*;
    use crate::{line_string_z, wkt};

    #[cfg(feature = "multithreading")]
    #[test]
    fn test_multithreading_linestring() {
        let multi: MultiLineStringZ<i32> = wkt! {
            MULTILINESTRING ((0 0 0, 2 0 0, 1 2 3, 0 0 0), (10 10,12 10,11 12,10 10))
        };
        let mut multimut: MultiLineStringZ<i32> = wkt! {
    ...
    fn test_iter() {
        let multi: MultiLineStringZ<i32> = wkt! {
            MULTILINESTRING Z ((0 0 0,2 0 0,1 2 0,0 0 0), (10 10 0,12 10 0,11 12 0,10 10 0))
        };

        let mut first = true;
        for p in &multi {
            if first {
                assert_eq!(p, &wkt! { LINESTRING Z (0 0 0,2 0 0,1 2 0,0 0 0) });
                first = false;
            } else {
                assert_eq!(p, &wkt! { LINESTRING Z (10 10 0,12 10 0,11 12 0,10 10 0) });
            }
        }
    ...
    #[test]
    fn test_iter_mut() {
        let mut multi = MultiLineStringZ::new(vec![
            line_string_z![(x: 0, y: 0, z: 0), (x: 2, y: 0, z: 0), (x: 1, y: 2, z: 0), (x:0, y:0, z:0)],
            line_string_z![(x: 10, y: 10, z: 0), (x: 12, y: 10, z: 0), (x: 11, y: 12, z: 0), (x:10, y:10, z:0)],
        ]);
        ...
        for p in &multi {
            if first {
                assert_eq!(
                    p,
                    &line_string_z![(x: 2, y: 2, z: 0), (x: 4, y: 2, z: 0), (x: 3, y: 4, z: 0), (x:2, y:2, z:0)]
                );
                first = false;
            } else {
                assert_eq!(
                    p,
                    &line_string_z![(x: 12, y: 12, z: 0), (x: 14, y: 12, z: 0), (x: 13, y: 14, z: 0), (x:12, y:12, z:0)]
                );
            }
        }
    }
    ...
    #[test]
    fn empty() {
        let empty = MultiLineStringZ::<f64>::empty();
