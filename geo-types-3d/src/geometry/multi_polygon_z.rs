...
impl<T: CoordNum, IP: Into<PolygonZ<T>>> From<Vec<IP>> for MultiPolygonZ<T> {
    fn from(x: Vec<IP>) -> Self {
        Self(x.into_iter().map(|p| p.into()).collect())
    }
}

impl<T: CoordNum, IP: Into<PolygonZ<T>>> FromIterator<IP> for MultiPolygonZ<T> {
    fn from_iter<I: IntoIterator<Item = IP>>(iter: I) -> Self {
        Self(iter.into_iter().map(|p| p.into()).collect())
    }
}

impl<T: CoordNum> IntoIterator for MultiPolygonZ<T> {
...
impl<'a, T: CoordNum> IntoIterator for &'a MultiPolygonZ<T> {
...
#[cfg(feature = "multithreading")]
impl<T: CoordNum + Send> IntoParallelIterator for MultiPolygonZ<T> {
    type Item = PolygonZ<T>;
    type Iter = rayon::vec::IntoIter<PolygonZ<T>>;
...
#[cfg(feature = "multithreading")]
impl<'a, T: CoordNum + Sync> IntoParallelIterator for &'a MultiPolygonZ<T> {
    type Item = &'a PolygonZ<T>;
    type Iter = rayon::slice::Iter<'a, PolygonZ<T>>;
...
#[cfg(feature = "multithreading")]
impl<'a, T: CoordNum + Send + Sync> IntoParallelIterator for &'a mut MultiPolygonZ<T> {
    type Item = &'a mut PolygonZ<T>;
    type Iter = rayon::slice::IterMut<'a, PolygonZ<T>>;
...
impl<T: CoordNum> MultiPolygonZ<T> {
    ...
    pub fn new(value: Vec<PolygonZ<T>>) -> Self {
        Self(value)
    }
...
mod approx_integration {
    ...
    impl<T> RelativeEq for MultiPolygonZ<T>
    where
    ...
    impl<T> AbsDiffEq for MultiPolygonZ<T>
    where
    ...
    impl<T> UlpsEq for MultiPolygonZ<T>
    where
...
mod test {
    use super::*;
    use crate::{polygon_z, wkt};

    #[test]
    fn test_iter() {
        let multi = MultiPolygonZ::new(vec![
            polygon_z![(x: 0, y: 0, z: 0), (x: 2, y: 0, z: 0), (x: 1, y: 2, z: 3), (x:0, y:0, z: 0)],
            polygon_z![(x: 10, y: 10, z: 10), (x: 12, y: 10, z:9), (x: 11, y: 12, z: 13), (x:10, y:10, z: 10)],
        ]);
    ...
    fn test_par_iter() {
        let multi = MultiPolygonZ::new(vec![
            polygon![(x: 0, y: 0, z: 0), (x: 2, y: 0, z: 0), (x: 1, y: 2, z: 3), (x:0, y:0, z: 0)],
            polygon![(x: 10, y: 10, z: 10), (x: 12, y: 10, z: 10), (x: 11, y: 12, z: 0), (x:10, y:10, z: 0)],
        ]);
        let mut multimut = MultiPolygonZ::new(vec![
            polygon![(x: 0, y: 0, z: 0), (x: 2, y: 0, z: 0), (x: 1, y: 2, z: 3), (x:0, y:0, z: 0)],
            polygon![(x: 10, y: 10, z: 10), (x: 12, y: 10, z: 10), (x: 11, y: 12, z: 13), (x:10, y:10, z: 10)],
        ]);
    ...
    #[test]
    fn test_iter_mut() {
        let mut multi = MultiPolygonZ::new(vec![
            polygon_z![(x: 0, y: 0, z: 0), (x: 2, y: 0, z: 0), (x: 1, y: 2, z: 3), (x:0, y:0, z: 0)],
            polygon_z![(x: 10, y: 10, z: 10), (x: 12, y: 10, z: 10), (x: 11, y: 12, z: 13), (x:10, y:10, z: 10)],
        ]);
        ...
        for p in &multi {
            if first {
                assert_eq!(
                    p,
                    &polygon_z![(x: 2, y: 2, z: 2), (x: 4, y: 2, z: 2), (x: 3, y: 4, z: 5), (x:2, y:2, z: 2)]
                );
                first = false;
            } else {
                assert_eq!(
                    p,
                    &polygon_z![(x: 12, y: 12, z: 12), (x: 14, y: 12, z: 12), (x: 13, y: 14, z: 15), (x:12, y:12, z: 12)]
                );
            }
        }
    }
    ...
    #[test]
    fn empty() {
        let empty = MultiPolygonZ::<f64>::empty();
