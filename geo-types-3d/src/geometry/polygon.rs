...
impl<T: CoordNum> PolygonZ<T> {
    ...
    /// );
    /// ```
    pub fn new(mut exterior: LineStringZ<T>, mut interiors: Vec<LineStringZ<T>>) -> Self {
        exterior.close();
    ...
    pub fn empty() -> Self {
        Self::new(LineStringZ::empty(), vec![])
    }
    ...
    /// );
    /// ```
    pub fn into_inner(self) -> (LineStringZ<T>, Vec<LineStringZ<T>>) {
    ...
    /// ```
    pub fn exterior(&self) -> &LineStringZ<T> {
        &self.exterior
    }
    ...
    where
        F: FnOnce(&mut LineStringZ<T>) -> Result<(), E>,
    {
        f(&mut self.exterior)?;
        self.exterior.close();
        Ok(())
    ...
    /// ```
    pub fn interiors(&self) -> &[LineStringZ<T>] {
        &self.interiors
    }
    ...
    ///
    /// [will be closed]: #linestring-closing-operation
    pub fn interiors_push(&mut self, new_interior: impl Into<LineStringZ<T>>) {
...
impl<T: CoordFloat + Signed> PolygonZ<T> {
...
//         Polygon::new(
//             vec![
//                 // Bottom face (z = r.min().z)
//                 (r.min().x, r.min().y, r.min().z),
//                 (r.max().x, r.min().y, r.min().z),
//                 (r.max().x, r.max().y, r.min().z),
//                 (r.min().x, r.max().y, r.min().z),
//                 (r.min().x, r.min().y, r.min().z),
//                 // Top face (z = r.max().z)
//                 (r.min().x, r.min().y, r.max().z),
//                 (r.max().x, r.min().y, r.max().z),
//                 (r.max().x, r.max().y, r.max().z),
//                 (r.min().x, r.max().y, r.max().z),
//                 (r.min().x, r.min().y, r.max().z),
//                 // Connect bottom to top (vertical edges)
//                 (r.min().x, r.min().y, r.min().z),
//                 (r.min().x, r.min().y, r.max().z),
//                 (r.max().x, r.min().y, r.min().z),
//                 (r.max().x, r.min().y, r.max().z),
...
impl<T: CoordNum> From<Triangle<T>> for PolygonZ<T> {
    fn from(t: Triangle<T>) -> Self {
        PolygonZ::new(vec![t.0, t.1, t.2, t.0].into(), Vec::new())
    }
}
...
mod approx_integration {
    ...
    impl<T> RelativeEq for PolygonZ<T>
    where
    ...
    impl<T> AbsDiffEq for PolygonZ<T>
    where
