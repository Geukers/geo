...
impl<T: CoordNum> Triangle<T> {
    ...
    pub fn to_array(&self) -> [CoordZ<T>; 3] {
        [self.0, self.1, self.2]
    }

    pub fn to_lines(&self) -> [LineZ<T>; 3] {
        [
            LineZ::new(self.0, self.1),
            LineZ::new(self.1, self.2),
            LineZ::new(self.2, self.0),
        ]
    }
    ...
    pub fn to_polygon(self) -> PolygonZ<T> {
        polygon_z![self.0, self.1, self.2, self.0]
    }
}
