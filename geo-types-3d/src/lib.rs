...
mod tests {
    ...
    fn type_test() {
        ...
        let p: PointZ<f32> = (0f32, 1f32, 2f32).into();
    ...
    fn convert_types() {
        ...
        let p2 = PointZ::try_from(g).unwrap();
        assert_eq!(p1, p2);
    }
    ...
    fn polygon_new_test() {
        ...
        let interiors = vec![LineStringZ::new(vec![
            coordZ! { x: 0.1, y: 0.1, z: 0. },
            coordZ! { x: 0.9, y: 0.9, z: 0. },
            coordZ! { x: 0.9, y: 0.1, z: 0. },
            coordZ! { x: 0.1, y: 0.1, z: 0. },
        ])];
        let p = PolygonZ::new(exterior.clone(), interiors.clone());
    ...
    #[test]
    fn iters() {
        let _: MultiPointZ<_> = vec![(0., 0., 0.), (1., 2., 0.)].into();
        let _: MultiPointZ<_> = vec![(0., 0., 0.), (1., 2., 0.)].into_iter().collect();

        let mut l1: LineStringZ<_> = vec![(0., 0., 0.), (1., 2., 0.)].into();
        assert_eq!(l1[1], coordZ! { x: 1., y: 2., z: 0. }); // index into linestring
        let _: LineStringZ<_> = vec![(0., 0., 0.), (1., 2., 0.)].into_iter().collect();

        // index mutably into a linestring
        l1[0] = coordZ! { x: 1., y: 1., z: 0. };
    ...
    fn test_coordinate_types() {
        ...
        let p: PointZ<i64> = PointZ::new(1_000_000, 0, 0);
    ...
    // #[test]
    // fn test_rects() {
    //     let r = Cube::new(coordZ! { x: -1., y: -1., z: 0. }, coordZ! { x: 1., y: 1., z: 0. });
    //     let p: PolygonZ<_> = r.into();
    //     assert_eq!(
    //         p,
    //         PolygonZ::new(
    //             vec![(-1., -1., 0.), (1., -1., 0.), (1., 1., 0.), (-1., 1., 0.), (-1., -1., 0.)].into(),
    //             vec![]
    //         )
    //     );
    // }
